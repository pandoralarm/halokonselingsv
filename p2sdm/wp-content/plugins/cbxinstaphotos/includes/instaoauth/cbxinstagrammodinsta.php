<?php
	 ...
	 * @subpackage CBXInstaPhotos/includes/instaoauth
	...
	require_once 'cbxcurlhttpclientmodinsta.php';


	class CBXInstagramModInsta {
		...
		protected $_endpointUrls = array(
			'authorize'                => 'https://api.instagram.com/oauth/authorize/?client_id=%s&redirect_uri=%s&response_type=%s&scope=basic+public_content',
			//'authorize'                => 'https://api.instagram.com/oauth/authorize/?client_id=%s&redirect_uri=%s&response_type=%s&scope=basic',
			'access_token'             => 'https://api.instagram.com/oauth/access_token',
			'user'                     => 'https://api.instagram.com/v1/users/%s/?access_token=%s',
			'user_feed'                => 'https://api.instagram.com/v1/users/self/feed?%s',
			//
			'user_recent'              => 'https://api.instagram.com/v1/users/self/media/recent/?access_token=%s&max_id=%s&min_id=%s&max_timestamp=%s&min_timestamp=%s',
			...
			'tags_recent'              => 'https://api.instagram.com/v1/tags/%s/media/recent?max_tag_id=%s&min_tag_id=%s&access_token=%s',
		...
		public function __construct( $config = null, $arrayResponses = false ) {
			...
			if ( empty( $config ) ) {
				throw new InstagramModTimelineException( 'Configuration params are empty or not an array.' );
			}
		}
		 ...
		 * @param string $method
		 */
		protected function _initHttpClient( $uri, $method = CurlHttpClientModInsta::GET ) {
			if ( $this->_httpClient == null ) {
				$this->_httpClient = new CurlHttpClientModInsta( $uri );
			} else {
		...
		protected function _setOauthToken() {
			$this->_initHttpClient( $this->_endpointUrls['access_token'], CurlHttpClientModInsta::POST );
		...
		public function createSubscription( $params ) {
			$this->_initHttpClient( $this->_endpointUrls['create_subscriptions'], CurlHttpClientModInsta::POST );
			$this->_httpClient->setPostParam( 'client_id', $this->_config['client_id'] );
			$this->_httpClient->setPostParam( 'client_secret', $this->_config['client_secret'] );
		...
		public function listSubscriptions() {
			...
			$this->_initHttpClient( $uri, CurlHttpClientModInsta::GET );
		...
		public function deleteSubscription( $params ) {
			...
			$uri = sprintf( $this->_endpointUrls['manage_subscriptions'], http_build_query( $getParams ) );
			$this->_initHttpClient( $uri, CurlHttpClientModInsta::DELETE );
		 ...
		 */
		public function getUserRecent( $maxId = '', $minId = '', $maxTimestamp = '', $minTimestamp = '', $count = null ) {
			$endpointUrl = sprintf( $this->_endpointUrls['user_recent'], $this->getAccessToken(), $maxId, $minId, $maxTimestamp, $minTimestamp, $count );
		...
		public function modifyUserRelationship( $id, $action ) {
			...
			$this->_initHttpClient( $endpointUrl, CurlHttpClientModInsta::POST );
		...
		public function deleteComment( $mediaId, $commentId ) {
			...
			$this->_initHttpClient( $endpointUrl, CurlHttpClientModInsta::DELETE );
	...
	class InstagramModTimelineException extends Exception {
	}
